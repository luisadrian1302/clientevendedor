...
export default function EliminarDescuentosSubproductos({ show, onClosevalue, onSave, toast, id }) {
    ...
    useEffect(() => {
        async function obtenerTodosLosproductos() {
            ...
            try {
                ...
                const { data } = await axios.get(`${URLAPI_SUBPRODUCT_SELLER}/getByUserandDescuento/${id}`, {
                    headers: {
    ...
    function toggleSelection(id) {
        ...
        document.querySelector("#removes" + id).classList.toggle('selected');
        // add or remove id
    }

    async function getSelectedCards() {
        const selectedCards = document.querySelectorAll('.removes.selected');
        const ids = Array.from(selectedCards).map(card => {
            let id = card.id.split("removes")[1]
            return id
    ...
    async function publicarSubproductosPeticion(ids) {
        ...
        try {
            ...
            const { data, status } = await axios.post(`${URLAPI_SUBPRODUCT_SELLER}/removeDescount/${id}`, payload, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            ...
            Swal.fire({
                title: "Error",
                text: "No se pudo remover el descuento en este producto",
                icon: "error"
            });
                    ...
                    <div className="modal-content  ">
                        ...
                        <div className="modal-body  ">
                            ...
                            <div className="container py-5">
                                <h3 className="mt-2 mb-4">Seleccione los sub productos que desea quitar el descuento</h3>
                                ...
                                <div className="row" id="cardContainer">
                                    ...
                                    {productActve.map(e => (
                                        ...
                                        <div className="col-lg-4 col-xl-3 col-md-6 col-sm-6" key={e}>
                                            <div className="card removes" id={"removes" + e.id} onClick={(element) => toggleSelection(e.id)}>
                                ...
                                <div className="mt-4" >
                                    <button className="btn btn-primary" onClick={(e) => getSelectedCards()}>Remover subproductos</button>
                                </div>
